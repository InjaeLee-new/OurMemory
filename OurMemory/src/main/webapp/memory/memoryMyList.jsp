...
<div id="wrap" class="wrap">
		<h1 class="title">나의 추억 모음전</h1>
		<div class="itemlist">
			<c:forEach var="memoryDTO" items="${list }">
				<div class="items">
					<a href="memoryMyView?memory_num=${memoryDTO.memory_num }&pg=1">
    	...
    	<div>
			<c:if test="${startPage > 3 }">
	           	[<a href="memoryMyList?pg=${startPage -1 }">이전</a>]
         	</c:if>
           	<c:forEach var="i" begin="${startPage }" end="${endPage }" step="1">  
        	 	[<a href="memoryMyList?pg=${i }">${i }</a>]
    	   	</c:forEach>
    	   	<c:if test="${endPage < maxPage }">
	    	   	[<a href="memoryMyList?pg=${endPage + 1 }">다음</a>]
			</c:if>	
		</div>
		<hr>
		<br>
